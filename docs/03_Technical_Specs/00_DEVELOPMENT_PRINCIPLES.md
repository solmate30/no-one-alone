...
| AI | LLM API (공급사 미확정) | 통화 흐름 처리 + 신호 분석·담당자 요약. MVP 진입 시 확정 |
| 전화·SMS | Twilio (글로벌) 또는 알리고·NCP (국내) | 아웃바운드 전화 발신 + SMS 수신 웹훅. MVP는 실연동 1개 또는 mock fallback |
| Auth | OmniOne CX (모바일 신분증), Open DID | OmniOne CX SDK |
| Blockchain | OmniOne Chain | 공식 SDK |
| Infra | Vercel (Frontend + API) | - |
| 정기 전화 크론 | Vercel Cron | - |
| CI/CD | GitHub Actions | - |

> **Open DID 인프라 주의**: `did-issuer-server`, `did-verifier-server`, `did-ledger-service-server`는 Java 21 + Docker Compose 전제다. Next.js 단일 스택만으로는 완결되지 않으며, 7월 초 기술 스파이크에서 `did-demo-server` + Ledger txId 1개 동작을 먼저 확인해야 한다.

### 폴백 (NestJS 분리 스택)

정기 전화 크론 잡, 장시간 통화 처리, Open DID 서버 사이드카, 또는 Vercel 제약으로 Next.js 단일 스택으로 해결이 어려운 경우 NestJS를 별도 백엔드로 분리한다.
...
# 전화·SMS Provider (Twilio 또는 알리고/NCP — MVP 진입 시 확정)
TELEPHONY_PROVIDER=twilio          # 'twilio' | 'aligo' | 'ncp'
TWILIO_ACCOUNT_SID=
TWILIO_AUTH_TOKEN=
TWILIO_FROM_NUMBER=
SMS_WEBHOOK_SECRET=                # Provider 웹훅 서명 검증용

# 전화번호 암호화 키 (target_phone_ciphertext 복호화용, AES-256-GCM)
...
│   ├── crisis/             — 위기도 스코어링 모듈
│   ├── soli/               — 솔이 전화·SMS·웹 채팅 분석 모듈
│   ├── targets/            — 정기 전화 대상자 등록 모듈
│   ├── welfare/            — 솔이 복지 매칭 모듈
...
| 항목 | 원칙 |
|:---|:---|
| 솔이 통화·SMS·웹 채팅 데이터 | AES-256 암호화 저장, 담당자만 요약 접근 (원문 불가) |
...
| 항목 | 목표 |
|:---|:---|
| 솔이 응답 지연 | MVP 3초 이내 목표 (Claude API 스트리밍 활용) |
...
| API | URL | 용도 |
|:---|:---|:---|
| 중앙부처복지서비스 | data.go.kr/data/15090532 | 솔이 복지 매칭 핵심 — 서비스명·지원대상·신청방법 |
...
- 솔이 활용 흐름: 대화 맥락 + 기본 조건(연령대·지역·가구 형태) + 선택형 상황 체크리스트를 정규화 → 조건 필터링 → 복지서비스 목록 반환 → 추천 카드 표시 → 서비스URL로 복지로/정부24 외부 신청 링크
- 개인 자격 확인(건강보험 자격득실 등): 공공 마이데이터 이용기관 심사 필요 → MVP 범위 제외, 파일럿 이후 정식 연계

### 7-5. Claude API (솔이 AI)

- 제공: Anthropic
- SDK: `@anthropic-ai/sdk` (npm)
- 문서: https://docs.anthropic.com
- MVP 구현 가능 기능:
  - SSE 스트리밍 응답 (솔이 응답 지연 3초 이내 목표)
